      ...
      <div class="main main-raised">
        <div class="section">
          <div class="container">
            ...
            <md-table v-model="comunas" md-sort="name" md-sort-order="asc" md-card md-fixed-header>
              <md-table-toolbar>
                
              </md-table-toolbar>
              <md-table-empty-state
                md-label="No existen elementos"
                md-description="Puedes crear acá nuevas Comunas para usarla en el sistema"
              ></md-table-empty-state>
              <md-table-row slot="md-table-row" slot-scope="{ item }">
                <md-table-cell md-label="ID" md-sort-by="id" md-numeric>{{ item.id }}</md-table-cell>
                <md-table-cell md-label="Nombre" md-sort-by="nombre">{{ item.nombre }}</md-table-cell>
                <md-table-cell md-label="id region" md-sort-by="id_region">{{ item.id_region }}</md-table-cell>
...
export default {
  ...
  methods: {
    ...
    newElement(model) {
      let vm = this
      comunaService.save(model).then(data => {
    ...
    updateElement(model) {
      let vm = this
      comunaService.update(model.id, model).then(data => {
        vm.comunas = data.body
