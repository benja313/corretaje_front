      ...
      <div class="main main-raised">
        <div class="section">
          <div class="container">
            ...
            <md-table v-model="bancos" md-sort="name" md-sort-order="asc" md-card md-fixed-header>
              <md-table-toolbar>
              ...
              <md-table-empty-state
                ...
                md-description="Puedes crear acá nuevos Bancos para usarlos en el sistema"
              ></md-table-empty-state>

              <md-table-row slot="md-table-row" slot-scope="{ item }">
                <md-table-cell md-label="ID" md-sort-by="id" md-numeric>{{ item.id }}</md-table-cell>
                <md-table-cell md-label="Nombre" md-sort-by="nombre">{{ item.nombre }}</md-table-cell>
                <md-table-cell md-label="Descripcion" md-sort-by="descripcion_zona">{{ item.rut }}</md-table-cell>
      ...
      <md-dialog :md-active.sync="showDialogUpdate" style="width: 70% !important;" >
        ...
        <form>
          ...
          <md-field class="md-form-group">
            <md-icon>text_format</md-icon>
            <label>Rut...</label>
            <md-input v-model="elementAux.rut" type="text" aria-required="required" />
          </md-field>
...
export default {
  ...
  methods: {
    ...
    newElement(model) {
      let vm = this
      bancoService.save(model).then(data => {
    ...
    updateElement(model) {
      let vm = this
      bancoService.update(model.id, model).then(data => {
        vm.bancos = data.body
