<template>
  <div>
    <div class="wrapper">
      ...
      >
        <h1 class="tim-note text-info">Administración de Restricciones</h1>
      </parallax>
          ...
          <div class="container">
            ...
            <md-table v-model="restricciones" md-sort="name" md-sort-order="asc" md-card md-fixed-header>
              ...
              <md-table-empty-state
                ...
                md-description="Puedes crear acá nuevas restricciónes para usarla en el sistema"
              ></md-table-empty-state>
...
export default {
  ...
  methods: {
    ...
    newElement(model) {
      let vm = this
      restriccionService.save(model).then(data => {
    ...
    updateElement(model) {
      let vm = this
      restriccionService.update(model.id, model).then(data => {
        vm.restricciones = data.body
    ...
    deleteElement(id) {
      let vm = this;
      restriccionService.destroy(id).then(
        data => {
          vm.restricciones = data.body
